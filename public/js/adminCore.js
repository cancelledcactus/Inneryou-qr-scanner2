...
function renderGrid(rooms) {
  ...
  for (const r of rooms) {
    ...
    const devOnline =
      (r.dev_online === 0) ? "Offline ❌" :
      (r.dev_online === 1) ? "Online ✅" : "Online —";
    const devBat = (r.dev_battery_pct !== null && r.dev_battery_pct !== undefined && `${r.dev_battery_pct}` !== "")
    ...
    const enabled =
      (r.dev_scanner_enabled === 0) ? "DISABLED" :
      (r.dev_scanner_enabled === 1) ? "ENABLED" : "ENABLED";
