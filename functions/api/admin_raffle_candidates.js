...
export async function onRequestGet({ request, env }) {
  ...
  const gradeNum = grade && /^\d{1,2}$/.test(grade) ? Number(grade) : null;

  // Eligible if distinct period_id count >= minp (period grouping happens in period_id itself)
  const sql = gradeNum !== null ? `
    ...
    GROUP BY student_id
    HAVING periods >= ?
    ORDER BY periods DESC, student_id ASC
    LIMIT 2000
  ` : `
    SELECT student_id, name, grade, COUNT(DISTINCT period_id) as periods
    FROM scans
  ...
  const rows = gradeNum !== null
    ? await env.DB.prepare(sql).bind(event_day, gradeNum, minp).all()
    : await env.DB.prepare(sql).bind(event_day, minp).all();

  return json({ ok:true, event_day, period_id, min_distinct_periods: minp, items: rows.results || [] });
}
